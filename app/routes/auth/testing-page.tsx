...
export async function clientAction({ serverAction, request }: Route.ClientActionArgs) {
  await sleep(1000);
  // la request es un readablestream, quiere decir que si se lee
  // una vez, no podrá ser leido una segunda, por lo que requerimos
  // clonarlo
...
export async function clientLoader({serverLoader}: Route.ClientLoaderArgs) {
  ...
  return {
    message: "Hola mundo desde el loader - Client",
    serverData
   };
}
...
export default function TestingPage({
  ...
  return (
    <div>
      ...
      <NavLink
      to="/auth/testing-args/ABC-123/Pablo/25"
      className={({isPending}) => 
        isPending
